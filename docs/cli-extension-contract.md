...
- **Auto-finish on zero findings (opt-in):** When **`auto_finish_zero_findings`** is enabled in config (`true`), via env (`STET_AUTO_FINISH_ZERO=1`), or per run with **`--auto-finish-zero`** on `stet start`, `stet run`, or `stet rerun`, the CLI calls the same finish path automatically after a successful review when the **active** findings count is zero (findings minus dismissed IDs). **`--dry-run`** reviews never auto-finish. When findings remain (including non-dismissed findings from earlier runs), the session stays open and explicit **`stet finish`** or the extension **Finish review** command is required. The VS Code setting **`stet.autoFinishZeroFindings`** passes **`--auto-finish-zero`** and clears the panel when the session ends with zero active findings.
...
| `auto_finish_zero_findings` / `STET_AUTO_FINISH_ZERO` | `false` | When true, auto-finish after `stet start` / `stet run` / `stet rerun` when active findings count is zero. Overridable per run with **`--auto-finish-zero`**. Not applied for **`--dry-run`**. |
...
**Git AI integration:** When **`refs/notes/ai`** exists (the repo uses [git-ai](https://github.com/git-ai-project/git-ai)), `stet stats volume` includes an optional `git_ai` object in JSON output with `commits_with_ai_note` and `total_ai_authored_lines`. Human output adds a "Git AI (refs/notes/ai)" section. The feature is auto-detected; no flag. Parsing follows [Git AI Standard v3.0.0](https://github.com/git-ai-project/git-ai/blob/main/specs/git_ai_standard_v3.0.0.md).
